...
if(!function_exists('get_products')){
	function get_products($id, $limit = 0)
	{
		...
		$data = (array) $CI->Product_model->child(get_lang_id_record($id, "product", $CI->data["default_lang"]->lang)->id, $limit);
		return $data;
	}
}
...
if(!function_exists('get_product')){
	function get_product($id)
	{
		global $CI;
		$CI->load->model('product/Product_model');
		$data = (array) $CI->Product_model->subrecord(get_lang_id_record($id, "product", $CI->session->userdata("lang"))->id);
		return $data;
	}
}
